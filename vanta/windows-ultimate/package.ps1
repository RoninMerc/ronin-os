...
$setup=Join-Path $out 'Ronin-Vanta-Windows-0.2.0-Setup.exe'
$info=Get-Item $setup
if($info.Length -lt 1000000){throw 'Installer is unexpectedly small'}
$install=Join-Path $env:RUNNER_TEMP 'Vanta Windows 020 Installed QA'
$profile=Join-Path $env:RUNNER_TEMP 'Vanta Windows 020 Private QA Data'
$env:VANTA_DATA_DIR=$profile
$env:DOTNET_ROOT=Join-Path $env:RUNNER_TEMP 'no-runtime-installed-here'
$env:DOTNET_MULTILEVEL_LOOKUP='0'
$record=[ordered]@{version='0.2.0';architecture='win-x64';platform=[System.Environment]::OSVersion.ToString();signing=(Get-AuthenticodeSignature $setup).Status.ToString();installerSha256=(Get-FileHash $setup).Hash;installerBytes=$info.Length}
Add-Type @'
using System; using System.Runtime.InteropServices;
public static class VantaSmokeWindow020 {
...
function Launch-Vanta([string]$label) {
 ...
 do {Start-Sleep -Milliseconds 100;$p.Refresh();if($p.HasExited){throw 'Installed application exited during startup'};$h=$p.MainWindowHandle;$r=New-Object VantaSmokeWindow020+Rect;if($h -ne [IntPtr]::Zero){[void][VantaSmokeWindow020]::GetWindowRect($h,[ref]$r)}} while(($h -eq [IntPtr]::Zero -or ($r.Right-$r.Left)-lt 850) -and (Get-Date)-lt $until)
 ...
 [void][VantaSmokeWindow020]::SetForegroundWindow($h)
 ...
 [void][VantaSmokeWindow020]::ShowWindow($h,6);Start-Sleep -Milliseconds 500;if(![VantaSmokeWindow020]::IsIconic($h)){throw 'Minimize failed'}
 [void][VantaSmokeWindow020]::ShowWindow($h,9);Start-Sleep -Milliseconds 500
...
if(!(Test-Path "$profile/vanta.sqlite") -or (Get-FileHash "$profile/account.key").Hash -ne $key){throw 'Uninstall erased private data'}
$record['install']='passed';$record['reinstall_and_data_preservation']='passed';$record['uninstall_and_data_preservation']='passed';$record['provider_testing']='No live provider calls; isolated offline profile'
...
Write-Output 'INSTALLER VERIFICATION PASSED: install, launch, minimize/restore, reinstall, private-data retention and uninstall.'
