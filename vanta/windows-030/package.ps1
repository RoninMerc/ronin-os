...
$installerSource=Join-Path $root 'ronin-vanta-windows/installer.iss'
...
$setup=Join-Path $out 'Ronin-Vanta-Windows-0.3.0-Setup.exe'
$info=Get-Item $setup
if($info.Length -lt 1000000){throw 'Installer is unexpectedly small'}
$install=Join-Path $env:RUNNER_TEMP 'Vanta Workstation Installed QA'
$profile=Join-Path $env:RUNNER_TEMP 'Vanta Workstation Installed QA Data'
$env:VANTA_DATA_DIR=$profile
$env:DOTNET_ROOT=Join-Path $env:RUNNER_TEMP 'no-runtime-installed-here'
$env:DOTNET_MULTILEVEL_LOOKUP='0'
$record=[ordered]@{version='0.3.0';architecture='win-x64';platform=[System.Environment]::OSVersion.ToString();signing=(Get-AuthenticodeSignature $setup).Status.ToString();installerSha256=(Get-FileHash $setup).Hash;installerBytes=$info.Length}
Add-Type @'
using System; using System.Runtime.InteropServices;
public static class VantaSmokeWindow030 {
...
function Launch-Vanta([string]$label) {
 ...
 do {Start-Sleep -Milliseconds 100;$p.Refresh();if($p.HasExited){throw 'Installed application exited during startup'};$h=$p.MainWindowHandle;$r=New-Object VantaSmokeWindow030+Rect;if($h -ne [IntPtr]::Zero){[void][VantaSmokeWindow030]::GetWindowRect($h,[ref]$r)}} while(($h -eq [IntPtr]::Zero -or ($r.Right-$r.Left)-lt 850) -and (Get-Date)-lt $until)
 if($h -eq [IntPtr]::Zero -or ($r.Right-$r.Left)-lt 850){Stop-Process $p -Force;throw 'Installed workstation never opened'}
 $record[$label+'_startup_ms']=$clock.Elapsed.TotalMilliseconds
 Start-Sleep -Seconds 2
 [void][VantaSmokeWindow030]::SetForegroundWindow($h)
 ...
 $cpu=$p.TotalProcessorTime.TotalMilliseconds;Start-Sleep -Seconds 8;$p.Refresh();$record[$label+'_idle_cpu_ms_over_8s']=$p.TotalProcessorTime.TotalMilliseconds-$cpu
 [void][VantaSmokeWindow030]::ShowWindow($h,6);Start-Sleep -Milliseconds 400;if(![VantaSmokeWindow030]::IsIconic($h)){throw 'Minimize failed'}
 [void][VantaSmokeWindow030]::ShowWindow($h,9);Start-Sleep -Milliseconds 500
...
$record['install']='passed';$record['reinstall_and_data_preservation']='passed';$record['uninstall_and_data_preservation']='passed';$record['provider_testing']='No live paid provider calls; offline fixture/profile verification'
...
Write-Output 'VANTA WORKSTATION INSTALLER VERIFICATION PASSED.'
